...
void CloudMapper::processByVer(PointCloudType::Ptr incloud,
                               PointCloudType::Ptr outcloud,
                               scanIndices &scanindices) {
  size_t cloudSize = incloud->points.size();

  std::vector<PointCloudType> laserCloudScans(_nScanRings);
  ...
  for (int i = 1; i < cloudSize; i++) {
    ...
    float angle =
        std::atan(incloud->points[i].z /
                  std::sqrt(incloud->points[i].x * incloud->points[i].x +
                            incloud->points[i].y * incloud->points[i].y));
...
void CloudMapper::processByIntensity(PointCloudType::Ptr incloud,
                                     PointCloudType::Ptr outcloud,
                                     scanIndices &scanindices) 
{
  size_t cloudSize = incloud->points.size();
  AINFO << "debug 14 " << endl;
  AINFO << "cloudSize " <<cloudSize << endl;
  AINFO << "_nScanRings " <<_nScanRings << endl;
  ...
  // 按 lidar 线数分段
  cloudSize = 0;
  AINFO << "getNumberOfScanRings() " <<getNumberOfScanRings() << endl;
...
void CloudMapper::processByOri(PointCloudType::Ptr incloud,
  ...
  for (size_t i = 1; i < incloud->points.size(); i++) {
    ...
    float ori =
        std::atan2(incloud->points[i].y, incloud->points[i].x) * 180 / M_PI;
    float ori_pre =
        std::atan2(incloud->points[i - 1].y, incloud->points[i - 1].x) * 180 /
        M_PI;
