...
protected:
  ...
  std::vector<std::vector<PlannerHNS::WayPoint>> m_RollOutsSelect;
  bool bRollOuts;

  std::vector<PlannerHNS::DetectedObject> m_PredictedObjects;
  bool bPredictedObjects;

  std::vector<geometry_msgs::Vector3> Inspec_points;
  ...
  int mode_state;
  std_msgs::Bool sweeper_status;
  std_msgs::Int32 auto_driving;  // khy add
  ...
  ros::Publisher pub_SweeperStatus;
  // khy add for app：发布一个话题，表明现在车辆处于自动驾驶
  ros::Publisher pub_IsAutoDriving;
  ...
  // subscribe welt order
	ros::Subscriber sub_mode_order;

  // inspection
  ros::Subscriber sub_inspection_point;
  ...
  void callbackGetBehaviorState(const geometry_msgs::TwistStampedConstPtr & msg);
	void callbackGetModeOrder(const std_msgs::Int32 msg);
  void callbackGetInspecionPoint(const geometry_msgs::Vector3 &msg);
