...
BehaviorGen::BehaviorGen()
{
  ...
  pub_LocalBasePath = nh.advertise<autoware_msgs::Lane>("base_waypoints_cut", 1,true);
  ...
  pub_ScenarioEnded = nh.advertise<std_msgs::Bool>("scenario_ended",1);
  ...
  sub_mode_order = nh.subscribe("/mode_order",1,&BehaviorGen::callbackGetModeOrder, this);
  sub_fitness_score = nh.subscribe("/ndt_stat",1,&BehaviorGen::callbackFitnessScore, this);
  ...
  sub_scenario = nh.subscribe("scenario_manager/scenario_cmd",1,&BehaviorGen::callbackGetScenario,this);
...
void BehaviorGen::UpdatePlanningParams(ros::NodeHandle& _nh)
{
  ...
  _nh.getParam("/op_behavior_selector/pose_match_threshold", m_BehaviorGenerator.pose_match_threshold);
...
void BehaviorGen::callbackGetModeOrder(const std_msgs::Int32 msg)
{
  m_BehaviorGenerator.mode_order = msg.data;
}
...
void BehaviorGen::callbackGetCANInfo(const autoware_can_msgs::CANInfoConstPtr &msg)
{
  ...
  // std::cout<<"current speed: "<<m_VehicleStatus.speed<<std::endl;
  UtilityHNS::UtilityH::GetTickCount(m_VehicleStatus.tStamp);
  bVehicleStatus = true;
}
...
void BehaviorGen::callbackFitnessScore(const autoware_msgs::NDTStat &msg){
  m_BehaviorGenerator.fitness_score = msg.score;
}
...
void BehaviorGen::callbackGetGlobalPlannerPath(const autoware_msgs::LaneArrayConstPtr& msg)
{
  if(msg->lanes.size() > 0 && bMap)
  {
    ...
    bool bOldGlobalPathSizeCheck = bOldGlobalPath;
    bool init_Global_path = false;
    
    if(!bOldGlobalPathSizeCheck){
      m_pre_GlobalPaths.clear();
      m_GlobalPaths.clear();
      init_Global_path = true;
    }
    ...
    for(unsigned int i = 0 ; i < msg->lanes.size(); i++)
    {
      ...
      else if(bOldGlobalPathSizeCheck){
        // not correctly !!! 
        bool double_check = PlannerHNS::PlanningHelpers::CompareTrajectories(m_temp_path, m_pre_GlobalPaths.at(i));
        std::cout<<"Is old Global Path? "<<double_check<<std::endl;
        if(!double_check){
          // m_GlobalPaths.clear();
          m_GlobalPaths.at(i) = m_temp_path;
        }
        bOldGlobalPath = bOldGlobalPath & double_check;
      }

    }
    m_pre_GlobalPaths = m_GlobalPaths;

    // 如果 接收到了 新的 op planner 轨迹，就表示 指定了新的 op planner终点，要重新规划
    if(!bOldGlobalPath)
    {
      bWayGlobalPath = true;
      ...
      for(unsigned int i = 0; i < m_GlobalPaths.size(); i++)
      {
        ...
        std::cout<<"m_GlobalPath: "<<i<<" ,"<<m_GlobalPaths.at(i).size()<<std::endl;
      }
    ...
    {
      std::cout<<"Received Old Path!"<<std::endl;
    }
  }
}
...
void BehaviorGen::callbackGetLocalTrajectoryCost(const autoware_msgs::LaneConstPtr& msg)
{
  ...
  m_TrajectoryBestCost.lane_index = static_cast<int>(msg->lane_id);
  
}

// 这里待更改，主要是考虑局部路径与全局轨迹的匹配。
// 将原本放在callbackGetGlobalPlannerPath()中的重置操作，放到这里来，因为在这个回调中，还有一步是局部轨迹和全局轨迹的匹配
    ...
    if(bWayGlobalPath && m_GlobalPaths.size() > 0)
    {
      if(m_GlobalPaths.at(0).size() > 0)
      {
        ...
        if(globalPathId_roll_outs == globalPathId)
        {
          ...
          // 重置操作：在op接收到新的终点重规划时，要重置。放在局部轨迹的回调中
          m_BehaviorGenerator.reset();
          ROS_INFO("Reset end status");
          // 是否发布end的标志位也要重置。并且要放在reset()之后，因为reset()里会对scenario_ended重置，这样避免了在SendLocalPlanningTopics()的最后，直接进入到发布end状态
          is_ended_published = false;
          resetFlag = true;
...
void BehaviorGen::SendLocalPlanningTopics()
{
  ...
  // 如果已经是END状态
  if(m_BehaviorGenerator.scenario_ended && !is_ended_published){
    std_msgs::Bool ended;
    autoware_msgs::DrivingStatus end_status;

    ended.data = m_BehaviorGenerator.scenario_ended;
    // mpc中有对该变量的判断，如果是2，则速度设置成0，即停车
    end_status.vehicle_status.data = 2;

    pub_ScenarioEnded.publish(ended);  // 将当前场景结束的标志位 反馈回 场景管理节点
    vehicle_status_pub_.publish(end_status);
...
void BehaviorGen::MainLoop()
{
  ...
  while (ros::ok())
  {
    ...
    else if (m_MapType == PlannerHNS::MAP_AUTOWARE && !bMap)  // 选择autoware格式的map
    {
      std::vector<UtilityHNS::AisanDataConnFileReader::DataConn> conn_data;;

      if(m_MapRaw.GetVersion()==2)  // 选择的是V2
      {
    ...
    // 有activate_scenario限制，只有在lane_driving场景的时候，才会进行局部op的规划，以及发布一些规划结果
    if(bNewCurrentPos && m_GlobalPaths.size()>0 && activate_scenario && resetFlag)
    {
      if(bNewLightSignal)
      {
      ...
      SendLocalPlanningTopics();  // 更新局部规划的 当前状态
...
void BehaviorGen::callbackGetScenario(const autoware_msgs::ScenarioCmd &msg){
  if(msg.scenario == 0){
    activate_scenario = true;
    ROS_INFO("receieve lane driving scenario in op_behavior_selector!");
  }
  else if(msg.mode == 1){
    resetFlag = false;
    activate_scenario = false;
    ROS_INFO("receieve parking scenario command, shutdown lane driving in op_behavior_selector!");
  }
