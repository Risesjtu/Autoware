...
void TrajectoryGen::callbackGetGlobalPlannerPath(const autoware_msgs::LaneArrayConstPtr& msg)
{
  if(msg->lanes.size() > 0)
  {
    std::cout<<"global path number: "<<msg->lanes.size()<<std::endl;
...
void TrajectoryGen::callbackGetScenario(const autoware_msgs::ScenarioCmd &msg){
  if(msg.scenario == 0){
    activate_scenario = true;
    ROS_INFO("receieve lane driving scenario in op_trajectory_generator!");
  }
  else if(msg.mode == 1){
    activate_scenario = false;
    ROS_INFO("receieve parking scenario command, shutdown lane driving in op_trajectory_generator!");
  }
