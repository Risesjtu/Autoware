    ...
    : nh_(""), pnh_("~"), my_position_ok_(false), my_velocity_ok_(false), my_steering_ok_(false)
{
  ...
  pub_gear_cmd_ = nh_.advertise<tablet_socket_msgs::gear_cmd>("gear_cmd", 0);
  ...
  sub_reverse_ = nh_.subscribe("/mpc_reverse", 1, &MPCFollower::callbackMPCReverse, this);
  sub_end_point_status_ = nh_.subscribe("/driving_status", 1, &MPCFollower::callbackEndPointStatus, this);
...
void MPCFollower::timerCallback(const ros::TimerEvent &te)
{
  ...
  {
    std::cout<<"vehicle model or qpsolver model is failed!"<<std::endl;
  ...
  if (!mpc_solved || end_point_status_ == 2)
  {
...
{
  // const bool sign_vx = is_reverse_? -1 : 1;
  ...
  if (!MPCUtils::calcNearestPoseInterp(ref_traj_, vehicle_status_.pose, nearest_pose, 
        nearest_index, dist_err, yaw_err, nearest_traj_time, is_reverse_))
  {
...
void MPCFollower::callbackMPCReverse(const std_msgs::Bool &msg){
  is_reverse_ = msg.data;
}

void MPCFollower::callbackEndPointStatus(const autoware_msgs::DrivingStatus &msg){
  end_point_status_ = msg.vehicle_status.data;
}

void MPCFollower::callbackRefPath(const autoware_msgs::Lane::ConstPtr &msg)
{
  current_waypoints_ = *msg;
  for(auto &p:current_waypoints_.waypoints)
  {
    p.twist.twist.linear.x = abs(p.twist.twist.linear.x);
  }
  ...
  /* calculate curvature */
  MPCUtils::calcTrajectoryCurvature(traj, curvature_smoothing_num_, prev_traj);
  prev_traj = traj;
...
void MPCFollower::publishTwist(const double &vel_cmd, const double &omega_cmd)
{
  ...
  const int sgn_vx = is_reverse_? -1 : 1;
  const int sgn_steer = is_reverse_? -2 : 1;
  // std::cout<<"[twist cmd]reverse status = "<<is_reverse_<<std::endl;
  geometry_msgs::TwistStamped twist;
  double vel_cmd_num = vel_cmd;
  twist.header.frame_id = "/base_link";
  twist.header.stamp = ros::Time::now();
  twist.twist.linear.x = vel_cmd_num * sgn_vx;
  twist.twist.linear.y = 0.0;
  twist.twist.linear.z = 0.0;
  twist.twist.angular.x = 0.0;
  twist.twist.angular.y = 0.0;
  // 实车需要乘以0.3,仿真不需要
  twist.twist.angular.z = amathutils::normalizeRadian(omega_cmd * sgn_steer)*0.3; // *0.3
  // twist.twist.angular.z = amathutils::normalizeRadian(omega_cmd * sgn_steer);
...
void MPCFollower::publishCtrlCmd(const double &vel_cmd, const double &acc_cmd, const double &steer_cmd)
{
  const int sgn_vx = is_reverse_? -1 : 1;
  const int sgn_steer = is_reverse_? -2 : 1;
  // std::cout<<"[ctrl cmd]reverse status = "<<is_reverse_<<std::endl;
  autoware_msgs::ControlCommandStamped cmd;
  double steer_cmd_num = steer_cmd;
  cmd.header.frame_id = "/base_link";
  cmd.header.stamp = ros::Time::now();
  cmd.cmd.linear_velocity = vel_cmd * sgn_vx;
  cmd.cmd.linear_acceleration = acc_cmd;
  cmd.cmd.steering_angle = amathutils::normalizeRadian(steer_cmd_num * sgn_steer)*0.3; //*0.3
