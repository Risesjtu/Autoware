                ...
                MPCTrajectory &prev_traj)
{
  ...
  /* calculate curvature by circle fitting from three points */
    ...
    if(2 * curvature_smoothing_num > traj_k_size){
    // geometry_msgs::Point p1, p2, p3;
    // for (unsigned int i = 2; i < traj_k_size - 2; ++i)
    // {
    //   p1.x = traj.x[i - 2];
    //   p2.x = traj.x[i];
    //   p3.x = traj.x[i + 2];
    ...
    // for(int i = 0; i < traj.k.size() - 1; ++i){
      // if(abs(traj.k.at(i) - traj.k.at(i+1)) > 0.3){
            // std::cout<<"here error!"<<std::endl;
            // const int offset = prev_traj.x.size() - traj_k_size;
            // const int front = offset / 2;
            // const int back = offset - front;
            // prev_traj.k.erase(prev_traj.k.end() - back, prev_traj.k.end());
            // prev_traj.k.erase(prev_traj.k.begin(), prev_traj.k.begin() + front);
            // traj.k = prev_traj.k;
            // break;
      // }
    // }
                                      ...
                                      const bool &is_reverse_)
{
  ...
  double my_yaw = tf2::getYaw(self_pose.orientation);

  if(is_reverse_){
    my_yaw = amathutils::normalizeRadian(my_yaw + M_PI);
  }
  ...
  for (uint i = 0; i < traj.size(); ++i)
  {
    ...
    // std::cout<<"my_yaw"<<my_yaw;
    // std::cout<<"    traj"<<traj.yaw[i]<<std::endl;
    if (fabs(err_yaw) < (M_PI / 3.0))
    {
      if (dist_squared < min_dist_squared)
      {
