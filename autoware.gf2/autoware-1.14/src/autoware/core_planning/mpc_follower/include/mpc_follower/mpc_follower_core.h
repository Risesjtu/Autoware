...
private:
  ...
  ros::Publisher pub_gear_cmd_;           //!< @brief topic publisher for gear command
  ...
  tablet_socket_msgs::gear_cmd gear_msg_;
  ...
  ros::Subscriber sub_reverse_;        
  ros::Subscriber sub_end_point_status_;  //!< @brief get stop status of vehicle
  ...
  void callbackMPCReverse(const std_msgs::Bool &msg);

  void callbackEndPointStatus(const autoware_msgs::DrivingStatus &msg);
};
