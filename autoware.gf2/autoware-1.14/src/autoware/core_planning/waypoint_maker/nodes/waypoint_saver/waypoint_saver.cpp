...
private:
  ...
  void displayVehicleMarker(geometry_msgs::Pose pose) const;
  ...
  ros::Publisher vehicle_range_pub_;
  ...
  //vehicle params
  double width_,length_,wheelBaseLength_;
};
...
WaypointSaver::WaypointSaver() : private_nh_("~")
{
  ...
  private_nh_.param<double>("width",width_,1);
  private_nh_.param<double>("length",length_,1);
  private_nh_.param<double>("wheelBaseLength",wheelBaseLength_,1);
  ...
  vehicle_range_pub_ = nh_.advertise<visualization_msgs::MarkerArray>("vehicle_range_marker",10,true);
}
...
void WaypointSaver::outputProcessing(geometry_msgs::Pose current_pose, double velocity) const
{
  ...
  else
  {
    ...
    if (distance > interval_)
    {
      ...
      displayVehicleMarker(current_pose);
      previous_pose = current_pose;
    }
  }
}
...
void WaypointSaver::displayVehicleMarker(geometry_msgs::Pose pose) const{
  ...
  marray.markers.push_back(marker);
  id++;
  vehicle_range_pub_.publish(marray);
}
