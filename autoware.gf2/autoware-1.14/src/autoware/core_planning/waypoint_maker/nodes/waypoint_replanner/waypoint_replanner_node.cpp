...
WaypointReplannerNode::WaypointReplannerNode() : pnh_("~"), is_first_publish_(true)
{
  ...
  {
    lane_pub_ = nh_.advertise<autoware_msgs::LaneArray>("/lane_waypoints_array_astar", 10, true);
  }
  ...
  sub_scenario = nh_.subscribe("scenario_manager/scenario_cmd",1,&WaypointReplannerNode::callbackGetScenario,this);
...
void WaypointReplannerNode::callbackGetScenario(const autoware_msgs::ScenarioCmd &msg){
  if(msg.scenario == 2){
    activate_scenario = true;
    ROS_INFO("receieve parking scenario in waypoint_loader_replan!");
  }
  else if(msg.mode == 1){
    activate_scenario = false;
