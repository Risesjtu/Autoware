...
// 检查：new_waypoints_（为了停避的新轨迹）的速度 和 原本轨迹速度 是否 突变太大，如果速度变化太大，就做一个平滑
...
// 在 当前位置 和 障碍物位置 间 设置一条 减速至停车 的轨迹
void VelocitySetPath::changeWaypointsForStopping(int stop_waypoint, int obstacle_waypoint, int closest_waypoint, double deceleration)
{
  ...
  // decelerate with constant deceleration
  // 将 closest_waypoint 和 stop_waypoint 之间的 轨迹点的速度设置为 递减的速度（根据deceleration）
  ...
  // 接着 将 stop_waypoint 和 obstacle_waypoint 之间的 轨迹点的速度设置为 0
...
void VelocitySetPath::callbackGetScenario(const autoware_msgs::ScenarioCmd &msg){
  if(msg.scenario == 1 || msg.scenario == 2){ // parking scenario
    activate_scenario_ = true;
    ROS_INFO("receieve lane driving scenario in velocity_set!");
  }
  else if(msg.mode == 1){ 
    activate_scenario_ = false;
