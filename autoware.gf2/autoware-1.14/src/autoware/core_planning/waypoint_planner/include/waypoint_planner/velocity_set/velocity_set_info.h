...
class VelocitySetInfo
{
 private:
  ...
  double pose_match_threshold_;      // when pose is not matched, then -> stop
 ...
 public:
  ...
  void locationCallback(const autoware_msgs::NDTStat &msg);
  ...
  double getPoseMatchThreshold() const
  {
    return pose_match_threshold_;
  }
