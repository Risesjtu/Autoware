...
#include "astar_search/rs_path.h"
...
public:
  ...
  // khy add:
  std::vector<bool> velocity_flag_;  // 用于记录 A*输出的全局轨迹中 每个轨迹点 的 速度 正负
...
private:
  ...
  bool AnalyticExpansions(const AstarNode* current_node, const AstarNode* goal_node,double &length, VectorVec4d &rs_path_with_velocity);
  void setRSPath(VectorVec4d &rs_path_with_velocity);
  // ros param
  ros::NodeHandle n_;
