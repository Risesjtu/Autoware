...
void TwistFilterNode::ctrlCmdCallback(const autoware_msgs::ControlCommandStampedConstPtr& msg)
{
  ...
  // std::cout<<"cmd.steering_angle in twist_filter is="<<msg->cmd.steering_angle<<std::endl;
  ...
  // std::cout<<"after limit ctrl_out.steering_angle in twist_filter is="<<ctrl_out.sa<<std::endl;
  // Publish lateral accel and jerk before smoothing
  ...
  // std::cout<<"after smooth ctrl_out.steering_angle in twist_filter is="<<ctrl_out.sa<<std::endl;
