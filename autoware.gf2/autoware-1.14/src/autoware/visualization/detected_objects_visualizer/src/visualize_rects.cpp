...
VisualizeRects::VisualizeRects()
{
  ...
  private_nh_.param<int>("left_ignore_line", left_ignore_line_, 200);
  private_nh_.param<int>("right_ignore_line", right_ignore_line_, 1720);
  private_nh_.param<int>("bottom_emergency_line", bottom_emergency_line_, 1050);
  ...
  const autoware_msgs::DetectedObjectArray::ConstPtr &in_objects)
{
  try
  {
    ...
    drawn_image = DrawIgnoreLine(drawn_image, left_ignore_line_, right_ignore_line_, bottom_emergency_line_);
...
cv::Mat VisualizeRects::DrawIgnoreLine(cv::Mat in_image, int left_horizon_line, int right_horizon_line, int bottom_vertical_line)
{ 
  int width = in_image.cols;   // 1920
  int height = in_image.rows;  // 1080

  cv::Point left_begin(left_horizon_line, 0);
  cv::Point left_end(left_horizon_line, height - 1);
  cv::Point right_begin(right_horizon_line, 0);
  cv::Point right_end(right_horizon_line, height - 1);
  cv::Point bottom_begin(0, bottom_vertical_line);
  cv::Point bottom_end(width - 1, bottom_vertical_line);

  cv::line(in_image, left_begin, left_end, cv::Scalar(0, 0, 255), 3, cv::LINE_8, 0);       // thickness=3, shift=0
  cv::line(in_image, right_begin, right_end, cv::Scalar(0, 0, 255), 3, cv::LINE_8, 0);
  cv::line(in_image, bottom_begin, bottom_end, cv::Scalar(0, 0, 255), 3, cv::LINE_8, 0);
