...
void geo_pos_conv::set_plane(int num) {
  ...
  {
    // lon_deg = 106;  // {你所在地点的经度(度)}
    // lon_min = 40;   // {你所在地点的经度(分)}
    // lat_deg = 26;   // {你所在地点的纬度(度)}
    // lat_min = 44.94;   // {你所在地点的纬度(分)}

    lon_deg = 26;  // {你所在地点的经度(度)}
    lon_min = 44.9314;   // {你所在地点的经度(分)}
    lat_deg = 106;   // {你所在地点的纬度(度)}
    lat_min = 40.0526;   // {你所在地点的纬度(分)}
  }
  else if(num == 126)
  {
  ...
  {
    // 龙马实际区域, 实际是25°2'36.8948N, 117°0'12.1496E
    // world基本上没有实际意义, 只是用来可视化, map是建图附近找的一个整数经纬度点
    // 每次建立新图, 先得到一个pcd点云地图, 然后确定此处的整数经纬度, 再运行定位, 记录gnss_pose和ndt_pose
    // 再运行extract_tf.py, 得到一个tf, 将这个tf作用到整个pcd点云地图, 得到新的地图, 就可以了, 这个地图是以如下整数经纬度为基准的地图
    // 另外, tf下world到map的tf.launch可以随便写, 要为了好一点可视化可以自己选择
    lon_deg = 25;
    lon_min = 2;
    lat_deg = 117;
    lat_min = 0;
  ...
  m_PLo = M_PI *
          (static_cast<double>(lat_deg) + static_cast<double>(lat_min) / 60.0) /
          180.0;
  m_PLato =
      M_PI *
      (static_cast<double>(lon_deg) + static_cast<double>(lon_min) / 60.0) /
      180;
}
