...
void updateCustomArea(std::map<Key<CustomArea>, CustomArea>& map, const CustomAreaArray& msg)
{
  map = std::map<Key<CustomArea>, CustomArea>();
  for (const auto& item : msg.data)
  {
    if (item.cid == 0)
      continue;
    map.insert(std::make_pair(Key<CustomArea>(item.cid), item));
  }
}
...
bool VectorMap::hasSubscribed(category_t category) const
{
  ...
  if (category & CUSTOM_AREA){
    if (custom_area_.empty())
      return false;
  }
  return true;
}
...
void VectorMap::registerSubscriber(ros::NodeHandle& nh, category_t category)
{
  ...
  if (category & CUSTOM_AREA){
    custom_area_.registerSubscriber(nh,"/vector_map_info/custom_area"); // "/vector_map_info/custom_area" is topic name
    custom_area_.registerUpdater(updateCustomArea);
  }
}
...
CustomArea VectorMap::findByKey(const Key<CustomArea>& key) const
{
  return custom_area_.findByKey(key);
}
...
std::vector<CustomArea> VectorMap::findByFilter(const Filter<CustomArea>& filter) const
{
  return custom_area_.findByFilter(filter);
}
...
void VectorMap::registerCallback(const Callback<CustomAreaArray>& cb)
{
  custom_area_.registerCallback(cb);
}
...
std::ostream& operator<<(std::ostream& os, const vector_map::CustomArea& obj)
{
  os << obj.cid << ","
     << obj.aid;
  return os;
...
std::istream& operator>>(std::istream& is, vector_map::CustomArea& obj)
{
  std::vector<std::string> columns;
  std::string column;
  while (std::getline(is, column, ','))
