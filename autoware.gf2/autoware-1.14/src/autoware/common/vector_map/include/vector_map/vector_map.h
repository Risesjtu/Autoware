...
// add by linkx
#include <vector_map_msgs/CustomArea.h>
#include <vector_map_msgs/CustomAreaArray.h>
...
//add by linkx
using vector_map_msgs::CustomArea;
using vector_map_msgs::CustomAreaArray;
...
private:
  ...
  Handle<CustomArea,CustomAreaArray> custom_area_;
...
public:
  ...
  CustomArea findByKey(const Key<CustomArea> & key) const;
  ...
  std::vector<CustomArea> findByFilter(const Filter<CustomArea> &filter) const;
  ...
  void registerCallback(const Callback<CustomAreaArray> &cb);
};
