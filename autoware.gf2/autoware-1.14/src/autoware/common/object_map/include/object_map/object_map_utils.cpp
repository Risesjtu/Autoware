...
namespace object_map
{
  ...
  // 只是将上面函数的WayArea改为了CUSTON_AREA
  void LoadRoadAreasFromCustomVectorMap(ros::NodeHandle& in_private_node_handle,
                                  std::vector<std::vector<geometry_msgs::Point>>& out_area_points)
  {
    vector_map::VectorMap vmap;
    vmap.subscribe(in_private_node_handle,
                   vector_map::Category::POINT | vector_map::Category::LINE | vector_map::Category::AREA |
                   vector_map::Category::CUSTOM_AREA, 10);

    std::vector<vector_map_msgs::CustomArea> way_areas =
        vmap.findByFilter([](const vector_map_msgs::CustomArea &way_area)
                          {
                            return true;
                          });
    ...
    if (way_areas.empty())
    {
      ROS_WARN_STREAM("No Custom area...");
      return;
    }
