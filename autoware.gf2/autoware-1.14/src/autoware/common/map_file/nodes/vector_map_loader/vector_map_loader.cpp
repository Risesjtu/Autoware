...
using vector_map::CustomAreaArray;
...
visualization_msgs::MarkerArray createCustomAreaMarkerArray(const VectorMap& vmap, Color color)
{
  visualization_msgs::MarkerArray marker_array;
  int id = 0;
  for (const auto& custom_area : vmap.findByFilter([](const CustomArea& custom_area){return true;}))
  {
    if (custom_area.aid == 0)
    {
      ROS_ERROR_STREAM("[createCustomAreaMarkerArray] invalid custom_area: " << custom_area);
      continue;
    }

    Area area = vmap.findByKey(Key<Area>(custom_area.aid));
    if (area.aid == 0)
    {
      ROS_ERROR_STREAM("[createCustomAreaMarkerArray] invalid area: " << area);
      continue;
    }

    visualization_msgs::Marker marker = createAreaMarker("custom_area", id++, color, vmap, area);
    if (isValidMarker(marker))
      marker_array.markers.push_back(marker);
    else
      ROS_ERROR_STREAM("[createCustomAreaMarkerArray] failed createAreaMarker: " << area);
  }
  return marker_array;
}
...
int main(int argc, char **argv)
{
  ...
  for (const auto& file_path : file_paths)
  {
    ...
    else if (file_name == "custom_area.csv")
    {
      category |= registerVectormapPortion<CustomArea, CustomAreaArray>(file_path, &custom_area_pub, "/vector_map_info/custom_area", Category::CUSTOM_AREA, &nh);
    }
    else
    {
  ...
  insertMarkerArray(marker_array, createCustomAreaMarkerArray(vmap, Color::YELLOW));
