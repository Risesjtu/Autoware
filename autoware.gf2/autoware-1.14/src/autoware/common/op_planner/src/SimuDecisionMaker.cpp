    ...
    const bool& bEmergencyStop)
{
  ...
  vector<geometry_msgs::Vector3> Inspec_points;
  std::vector<std::vector<WayPoint>> rollOuts_select; // can not use in the function

  PlannerHNS::TrajectoryCost tc = m_TrajectoryCostsCalculator.DoOneStepStatic(m_RollOuts, rollOuts_select,m_TotalPath, state,  m_params, m_CarInfo, vehicleState, objects, Inspec_points,sweeper_status);
