...
TrajectoryCost TrajectoryDynamicCosts::DoOneStepDynamic(const vector<vector<WayPoint> >& rollOuts, vector<vector<WayPoint>> &rollOuts_select,
    const vector<vector<WayPoint>>& globalPaths, const WayPoint& currState,
    const PlanningParams& params, const CAR_BASIC_INFO& carInfo, const VehicleState& vehicleState,
    const std::vector<PlannerHNS::DetectedObject>& obj_list, std::vector<geometry_msgs::Vector3> &Inspec_points,
    std_msgs::Bool &sweeper_status, const int& mode_order,const int& iCurrentIndex)
{
    // 截取当前考虑部分
  vector<WayPoint> totalPaths;
  //std::cout<<"globalPaths size: "<<globalPaths.size()<<std::endl;
  rollOuts_select.clear();

  // normal mode, sweeper status 
  if(mode_order > 0){
    sweeper_status.data = true;
  }
  else{
    sweeper_status.data = false;
  }

  if(globalPaths.size()>=2 )
  {
    //std::cout<<"rollOuts.size(): "<<rollOuts.size()<<std::endl;
    int mid_index = rollOuts.size()/2;
    if(mode_order == 1)
    {
      totalPaths = globalPaths.at(1);
      rollOuts_select.assign(rollOuts.begin()+mid_index,rollOuts.end());
    }
    else
    {
      totalPaths = globalPaths.at(0);
      rollOuts_select.assign(rollOuts.begin(),rollOuts.begin()+mid_index);
    }
    // std:cout<<"rollOuts.size(): "<<rollOuts.size()<<std::endl;
  }
  else{
      totalPaths = globalPaths.at(0);
      rollOuts_select = rollOuts;
    }
  ...
  int currIndex = -1;
  if(iCurrentIndex >=0 && iCurrentIndex < rollOuts_select.size())
    currIndex  = iCurrentIndex;
  else
    currIndex = GetCurrentRollOutIndex(totalPaths, currState, params);

  InitializeCosts(rollOuts_select, params);
  ...
  CalculateLateralAndLongitudinalCostsDynamic(obj_list, rollOuts_select, totalPaths,  currState, params, carInfo, vehicleState, critical_lateral_distance, critical_long_front_distance, critical_long_back_distance);
  ...
  {
    // linkx add 0223
    if(ic > m_TrajectoryCosts.size()/2)
    continue;
  ...
  //   smallestIndex = params.rollOutNumber/2;
    ...
    // linkX add 0223
  int middle_index = params.rollOutNumber/2;
  if(!m_TrajectoryCosts.at(middle_index).bBlocked && smallestIndex >=0)
    smallestIndex = middle_index;

  if (currIndex != smallestIndex)
  {
    ROS_INFO("The best trajectory index %d",smallestIndex);
  }
  ...
  if(mode_order == 2)
  {
    ...
    {
      WayPoint p;
      RelativeInfo inspec_info;
      p.pos.x = Inspec_points[0].x;
      p.pos.y = Inspec_points[0].y;
      p.pos.z = Inspec_points[0].z;
      PlanningHelpers::GetRelativeInfo(totalPaths, p, inspec_info);
      
      // Choose a more radical path
      int target_closet_idx = 0;
      if(fabs(inspec_info.perp_distance)<carInfo.width/5){
        target_closet_idx = params.rollOutNumber/2;
      }
      else{
        double relative_index = inspec_info.perp_distance/params.rollOutDensity;
        if(relative_index<0)
          target_closet_idx = params.rollOutNumber/2 + floor(relative_index);
        else 
          target_closet_idx = params.rollOutNumber/2 + ceil(relative_index);
      }

      if(target_closet_idx < 0)
        target_closet_idx = 0;
      else if(target_closet_idx > params.rollOutNumber)
        target_closet_idx = params.rollOutNumber;
     RelativeInfo obj_info;
     PlanningHelpers::GetRelativeInfo(totalPaths, currState, obj_info);
      double longitudinalDist = PlanningHelpers::GetExactDistanceOnTrajectory(totalPaths, obj_info, inspec_info);
      if(inspec_info.iFront == 0 && longitudinalDist > 0)
        longitudinalDist = -longitudinalDist;
      cout<<"距离下一个垃圾的距离="<<longitudinalDist<<endl;
      if(longitudinalDist<10)
      {
        if(!m_TrajectoryCosts.at(target_closet_idx).bBlocked)
        {
            if(target_closet_idx >= 0)
            {
              bestTrajectory = m_TrajectoryCosts.at(target_closet_idx);
              cout<<"当前选择道路为："<<target_closet_idx<<endl;
            }
        }
      }
      if(longitudinalDist<-carInfo.length)
      {
        Inspec_points.erase(Inspec_points.begin());
      }
    }
    else{
       sweeper_status.data = false;
...
TrajectoryCost TrajectoryDynamicCosts::DoOneStepStatic(vector<vector<WayPoint> >& rollOuts,vector<vector<WayPoint>> &rollOuts_select,
    const vector<vector<WayPoint>>& globalPaths, const WayPoint& currState,
    const PlanningParams& params, const CAR_BASIC_INFO& carInfo, const VehicleState& vehicleState,
		const std::vector<PlannerHNS::DetectedObject>& obj_list, std::vector<geometry_msgs::Vector3> &Inspec_points,
    std_msgs::Bool & sweeper_status,const int& mode_order, const int& iCurrentIndex)
{
  ...
  int currIndex = params.rollOutNumber/2 + static_cast<int>(round(obj_info.perp_distance/params.rollOutDensity));
  ...
  if(rollOuts_select.size()>0)
  {
    TrajectoryCost tc;
    int centralIndex = params.rollOutNumber/2;
    tc.lane_index = 0;
    for(unsigned int it=0; it< rollOuts_select.size(); it++)
    {
      tc.index = it;
      ...
      if(rollOuts_select.at(it).size() > 0)
          tc.lane_change_cost = rollOuts_select.at(it).at(0).laneChangeCost;
  ...
  for(unsigned int io=0; io<obj_list.size(); io++)
  {
    for(unsigned int icon=0; icon < obj_list.at(io).contour.size(); icon++)
    {
      ...
      p.pos.a = obj_list.at(io).center.pos.a;
  ...
  CalculateLateralAndLongitudinalCostsStatic(m_TrajectoryCosts, rollOuts_select, totalPaths, currState, m_AllContourPoints, params, carInfo, vehicleState);
  ...
  for(unsigned int ic = 0; ic < m_TrajectoryCosts.size(); ic++)
  {
    ...
    //coutinspection_state << m_TrajectoryCosts.at(ic).ToString();
    if(!m_TrajectoryCosts.at(ic).bBlocked && m_TrajectoryCosts.at(ic).cost < smallestCost)
    {
      smallestCost = m_TrajectoryCosts.at(ic).cost;
      smallestIndex = ic;
  ...
  // std::cout<<"the best local path index: "<<smallestIndex<<std::endl; 
    ...
    const CAR_BASIC_INFO& carInfo, const VehicleState& vehicleState)
{
  ...
  double slide_distance = vehicleState.steer * ratio_to_angle; // slide_distance 可认为是车辆当前转角下可以往侧边行驶的距离
...
// 考虑当车辆有转角的时候，生成带有转向的边界框
  ...
  if(rollOuts.size() > 0 && rollOuts.at(0).size()>0)
  {
    ...
    for(unsigned int it=0; it< rollOuts.size(); it++)
    {
      ...
      {
        // contourPoints.at(icon).id 表示该边界点属于的障碍物，id>=0
...
//          if(close_in_percentage <= 0 || close_in_percentage > 1) close_in_percentage = 1;
        ...
        if(lateralDist-0.2 <= critical_lateral_distance
            && longitudinalDist >= -carInfo.length/0.9
            && longitudinalDist < params.minFollowingDistance){
            double a = UtilityHNS::UtilityH::AngleBetweenTwoAnglesPositive(contourPoints.at(icon).pos.a, currState.pos.a)/M_PI;
            if (contourPoints.at(icon).v > 1.0  && a < 0.5 ){
               trajectoryCosts.at(iCostIndex).bBlocked = false;
               ROS_INFO("current v is %f, obstacle v is %f, diff yaw is %f", currState.v,contourPoints.at(icon).v, a );
            }
            else 
            {
               trajectoryCosts.at(iCostIndex).bBlocked = true;
...
void TrajectoryDynamicCosts::CalculateTransitionCosts(vector<TrajectoryCost>& trajectoryCosts, const int& currTrajectoryIndex, const PlanningParams& params)
{
  ...
  // for(int ic=0;ic<trajectoryCosts.size();ic++)
  // {
  //   std::cout<<"index: "<<ic<<" transition_cost: "<<trajectoryCosts.at(ic).transition_cost<<std::endl;
  // }
}
  ...
  for(unsigned int i=0; i < obj_list.size(); i++)
  {
    ...
    else
    {
      ...
      for(unsigned int icon = 0; icon < obj_list.at(i).contour.size(); icon++)
      {
        ...
        {
          ROS_WARN("The obstacle %s is inside safety Polygon!",obj_list.at(i).label);
          for(unsigned int it=0; it< rollOuts.size(); it++)
