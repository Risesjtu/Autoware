...
TrajectoryCost DoOneStepStatic(vector<vector<WayPoint> >& rollOuts, vector<vector<WayPoint>> &rollOuts_select,
    ...
    const PlanningParams& params, const CAR_BASIC_INFO& carInfo, const VehicleState& vehicleState,
		const std::vector<PlannerHNS::DetectedObject>& obj_list, std::vector<geometry_msgs::Vector3> &Inspec_points,std_msgs::Bool &sweeper_status,
    const int& mode_order=0,const int& iCurrentIndex=-1);

TrajectoryCost DoOneStepDynamic(const vector<vector<WayPoint> >& rollOuts,vector<vector<WayPoint>> &rollOuts_select,
    const vector<vector<WayPoint>>& globalPaths, const WayPoint& currState,
    const PlanningParams& params, const CAR_BASIC_INFO& carInfo, const VehicleState& vehicleState,
    const std::vector<PlannerHNS::DetectedObject>& obj_list, vector<geometry_msgs::Vector3> &Inspec_points,std_msgs::Bool &sweeper_status,
    const int& mode_order=0,const int& iCurrentIndex=-1);
