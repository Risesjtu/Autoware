...
static geometry_msgs::PoseStamped prev_ndt_pose_msg;
...
static geometry_msgs::TwistStamped stop_estimate_twist_msg;
...
static double _pose_match_threshold = 25.0;
...
static bool judgeLocalization()
{
  if (fitness_score <= _pose_match_threshold) {
...
static void gnss_callback(const geometry_msgs::PoseStamped::ConstPtr& input)
{
  ...
  current_gnss_pose.z = 0;//input->pose.position.z;
  ...
  if (map_loaded == 1 && init_pos_set == 1)
  {
    ...
    */
    ...
    if (judgeLocalization()) {
      ndt_pose_pub.publish(ndt_pose_msg);  // /ndt_pose会在vel_pose_connect中以/current_pose话题发布出去，作为后续规划用到的位姿
      prev_ndt_pose_msg = ndt_pose_msg;
    }
    else {
      ndt_pose_pub.publish(prev_ndt_pose_msg);
    }
    
    // ndt_pose_pub.publish(ndt_pose_msg);  // original!
    // current_pose is published by vel_pose_mux
    ...
    if (judgeLocalization()) {
      ...
      stop_estimate_twist_msg.header.stamp = current_scan_time;
      stop_estimate_twist_msg.header.frame_id = "/base_link";
      stop_estimate_twist_msg.twist.linear.x = 0.0;
      stop_estimate_twist_msg.twist.linear.y = 0.0;
      stop_estimate_twist_msg.twist.linear.z = 0.0;
      stop_estimate_twist_msg.twist.angular.x = 0.0;
      stop_estimate_twist_msg.twist.angular.y = 0.0;
      stop_estimate_twist_msg.twist.angular.z = 0.0;
      estimate_twist_pub.publish(stop_estimate_twist_msg);
    }
...
int main(int argc, char** argv)
{
  ...
  private_nh.getParam("pose_match_threshold", _pose_match_threshold);
