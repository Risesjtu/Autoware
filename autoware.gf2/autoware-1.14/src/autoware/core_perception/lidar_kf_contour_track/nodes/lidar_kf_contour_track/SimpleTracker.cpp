...
void SimpleTracker::MergeObjectAndTrack(KFTrackV& track, PlannerHNS::DetectedObject& obj)
{
  ...
  if (track.obj.label == "unknown"){
    track.obj = obj;   // ljc add: 这里update的时候即便一开始检测出car, 但是后续为unknown则会被覆盖, 所以标签不变?
  }
  else{
    std::string label_ = track.obj.label;
    track.obj = obj;
    if (obj.label == "unknown"){
      track.obj.label = label_;   // 如果原先有融合后的标签, 则若新标签为未知, 则保留原标签; 新标签为其他类别, 可能是之前类别识别有误, 则进行更新; 这样处理对于静态障碍物应该没问题
      std::cout << "keep origin label " << track.obj.label << "\n";
    }
  }
