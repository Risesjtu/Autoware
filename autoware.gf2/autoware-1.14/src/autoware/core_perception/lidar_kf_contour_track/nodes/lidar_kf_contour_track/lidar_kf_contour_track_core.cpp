...
ContourTracker::ContourTracker()
{
  ...
  sub_cloud_clusters     = nh.subscribe("/detection/fusion_tools/objects", 1, &ContourTracker::callbackGetCloudClusters, this);  // ljc add: 已重写range_vision_fusion, 其输出为CloucClusterArray, 含有标签的cluster传给kf
  sub_current_pose     = nh.subscribe("/current_pose",   1, &ContourTracker::callbackGetCurrentPose,   this);

  pub_AllTrackedObjects   = nh.advertise<autoware_msgs::DetectedObjectArray>("/lidar_kf_track/objects", 1);
...
void ContourTracker::callbackGetCloudClusters(const autoware_msgs::CloudClusterArrayConstPtr &msg)
{
  ...
  {
    // msg cluster中的convex hull和bbox在欧式聚类中不会被转到output frame下, cloud和centroid会转, 不过kf只用到了后者, 并且前者丢弃了, 所以没问题
...
//  cout << endl;
  ...
  pub_AllTrackedObjects.publish(m_OutPutResults);   // kf输出的tracked_objects的pose已经是在map坐标系下了, 不同于欧式聚类无视output_frame的只在velodyne下
}
