...
namespace darknet
{
    ...
    std::vector< RectClassScore<float> > Yolo3Detector::forward(image& in_darknet_image)
    {
        ...
        detection *darknet_detections = get_network_boxes(&darknet_network_, darknet_network_.w, darknet_network_.h, min_confidence_, .5, NULL, 0, &nboxes, 0);
        ...
        std::vector<float> min_score_list_(num_classes, min_confidence_);  // 将人和车辆等置信度提高点
        min_score_list_[0] = 0.6;
        min_score_list_[1] = 0.5;
        min_score_list_[2] = 0.6;
        min_score_list_[3] = 0.4;
        min_score_list_[5] = 0.5;
        min_score_list_[7] = 0.5;
        // min_score_list_[39] = 0.2;
        min_score_list_[46] = 0.2;
        ...
        for (int i = 0; i < nboxes; i++)
        {
            ...
            int class_id = -1;
            float score = min_confidence_;
            //find the class
            for(int j = 0; j < num_classes; ++j){
                if (j == 0 || j == 1 || j == 2 ||     // 0: person, 1: bicycle, 2: car
                    j == 3 || j == 5 || j == 7 ||     // motorbike, bus, truck
                    j == 15 || j == 16 || j == 26 ||  // cat, dog, handbag
                    j == 39 || j == 41 || j == 46 ||  // bottle, cup, banana
                    j == 47 || j == 49 || j == 79)    // apple, orrange, toothbrush
                {
                    if (darknet_detections[i].prob[j] >= score && darknet_detections[i].prob[j] > min_score_list_[j]){
                        class_id = j;
                        score = darknet_detections[i].prob[j];
                    }
                }
