...
void do_nms_sort(detection *dets, int total, int classes, float thresh)
{
    ...
    for (k = 0; k < classes; ++k) {
        for (i = 0; i < total; ++i) {
            dets[i].sort_class = k;   // 先排序类别0, 再排序类别1
        }
        qsort(dets, total, sizeof(detection), nms_comparator_v3);
        for (i = 0; i < total; ++i) {   // 遍历所有的bbox
            //printf("  k = %d, \t i = %d \n", k, i);
            if (dets[i].prob[k] == 0) continue;   // 如果当前这个bbox的这个类别的概率为0, 直接continue, 访问下一个bbox(这里是0是因为它们会被IoU处理置零)
            box a = dets[i].bbox;
            for (j = i + 1; j < total; ++j) {
                box b = dets[j].bbox;
                if (box_iou(a, b) > thresh) {
                    dets[j].prob[k] = 0;    // 遍历该bbox后面的所有bbox(因为它们的score是降序的), 当IoU较大时,取score更大的那个, 即当前bbox, 而将后面的prob置为0, 并由此看出只会对当前类别的物体nms, 不同类别物体不会被nms消去
                }
            }
        }
    }
