                                      ...
                                      autoware_msgs::DetectedObjectArray& transformed_input)
{
  ...
  for (auto const &cluster: input.clusters)
  {
    geometry_msgs::Pose out_pose = getTransformedPose(cluster.bounding_box.pose, local2global_);
    

    autoware_msgs::DetectedObject dd;
    dd.header = input.header;

    dd.label = cluster.label;
    dd.score = cluster.score;
    dd.space_frame = input.header.frame_id;            
    dd.pose = out_pose;     
 
    dd.dimensions = cluster.dimensions;      // 与frame无关
    dd.pointcloud = cluster.cloud;           // 会转到output frame
    dd.convex_hull = cluster.convex_hull;    // 不会转到output frame, 仍为velodyne下
    dd.valid = true;
                           ...
                           autoware_msgs::DetectedObjectArray& detected_objects_output)
{
  ...
  for (size_t i = 0; i < targets_.size(); i++)
  {
    ...
    if (!targets_[i].is_static_ && targets_[i].is_stable_)   // 非静态并且稳定
    {
                        ...
                        autoware_msgs::DetectedObjectArray& detected_objects_output)
{
  ...
  if (!init_)
  {
    ...
    makeOutput(input, matching_vec, detected_objects_output);   // 初始化中没有用到matching_vec, 这里还全是false
    return;
  }
  ...
  // start UKF process
  for (size_t i = 0; i < targets_.size(); i++)   // target_是track住的物体, 和input作匹配; 未匹配上的input会新添加到target_中
  {
    ...
    // prevent ukf not to explode
    if (targets_[i].p_merge_.determinant() > prevent_explosion_threshold_ ||   // determinant: 行列式
  ...
  staticClassification();    // 用于确认是否是static

  // making output for visualization
  makeOutput(input, matching_vec, detected_objects_output);    // detected_objects_output是最终要发布出去的

  // remove unnecessary ukf object
  removeUnnecessaryTarget();    // 把die状态的物体去除
}
