...
void publishDetectedObjects(const autoware_msgs::CloudClusterArray &in_clusters)
{
  ...
  for (size_t i = 0; i < in_clusters.clusters.size(); i++)
  {
    ...
    detected_object.space_frame = in_clusters.header.frame_id;            // 会转到output frame
    detected_object.pose = in_clusters.clusters[i].bounding_box.pose;     // 不会转到output frame, 仍为velodyne下
    // std::cout << detected_object.pose.position.x << "\n"; 
    detected_object.dimensions = in_clusters.clusters[i].dimensions;      // 与frame无关
    detected_object.pointcloud = in_clusters.clusters[i].cloud;           // 会转到output frame
    detected_object.convex_hull = in_clusters.clusters[i].convex_hull;    // 不会转到output frame, 仍为velodyne下
                          ...
                          const std::string &in_target_frame, const std_msgs::Header &in_header)
{
  if (in_target_frame != in_header.frame_id)
  {
    ...
    for (auto i = in_clusters.clusters.begin(); i != in_clusters.clusters.end(); i++)
    {
      ...
      try
      {
        _transform_listener->lookupTransform(in_target_frame, _velodyne_header.frame_id, ros::Time(),
                                             *_transform);
        pcl_ros::transformPointCloud(in_target_frame, *_transform, i->cloud, cluster_transformed.cloud);   // 问题2: 在range_vision_fusion中查camera到指定坐标系的tf只查一次, 去除此限制后发现抖动很大
        ...
        cluster_transformed.label = i->label;

        if(_pose_estimation)
        {
          cluster_transformed.bounding_box.pose.orientation = i->bounding_box.pose.orientation;   // range_vision_fusion实际所需要的就是这里的boundingbox的pose, 但是这里没有对这个位置作坐标变换
        }
        else
        {
    ...
    publishDetectedObjects(clusters_transformed);   // ljc add: 如果要实现range vision fusion中要求velodyne的输出以及lidar kf中要求map下的输出, 只需要把这行的clusters_transformed改成in_clusters即可
                                                    // 但是考虑到后续可能的扩展性, 还是在range vision fusion中作改动
  } else
  {
    ...
    for (unsigned int i = 0; i < final_clusters.size(); i++)
    {
      *out_cloud_ptr = *out_cloud_ptr + *(final_clusters[i]->GetCloud());
      ...
      if (final_clusters[i]->IsValid())
      {
        ...
        cloud_cluster.label = "unknown";  // ljc add: 为了后续区别range_vision_fusion使用, 欧式聚类统一为unknown类别
...
void removeFloor(const pcl::PointCloud<pcl::PointXYZ>::Ptr in_cloud_ptr,
                 ...
                 pcl::PointCloud<pcl::PointXYZ>::Ptr out_onlyfloor_cloud_ptr, float in_max_height = 0.1,
...
void velodyne_callback(const sensor_msgs::PointCloud2ConstPtr& in_sensor_cloud)
{
  ...
  if (!_using_sensor_cloud)
  {
    ...
    if (_remove_ground)   // 输入是来自ray_ground_filter的points_no_ground, 所以不需要再去除地面
    {
