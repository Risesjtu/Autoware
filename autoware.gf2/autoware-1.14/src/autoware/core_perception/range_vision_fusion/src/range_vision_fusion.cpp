  ...
  { 
    ...
    if (!used_vision_detections[i])
    {
      ...
      { 
        ...
        if (in_vision_detections->objects[i].label == "bottle" || 
            in_vision_detections->objects[i].label == "banana" || 
            in_vision_detections->objects[i].label == "cup" || 
            in_vision_detections->objects[i].label == "toothbrush")
        {
          cv::Point2i bottem_point_2d(int(in_vision_detections->objects[i].x + in_vision_detections->objects[i].width / 2), int(in_vision_detections->objects[i].y + in_vision_detections->objects[i].height));
          tf::Vector3 bottem_point_3d = ProjectPoint(bottem_point_2d);   // 转到base_link下 
          ...
          ROS_INFO_STREAM("detect " << in_vision_detections->objects[i].label << " and its pose x: " << position_x << " pose y: " << position_y);

          if (position_x > 15 || position_x < 0 || position_y > 4 || position_y < -4)  // 如果预测出来的深度过于远, 认为不准, 40m开外也不需要太关心, 忽略, 若深度估计为负, 则可能是由于过于接近图像中心导致, 前置相机不可能如此, 忽略
          {
            continue;
          }
          
          if (closest_dist > position_x * position_x + position_y * position_y)
          {
            geometry_msgs::Pose pose;
            geometry_msgs::Vector3 dimensions;
            ...
            detected_cluster.label = "bottle";   // 统一发布为Bottle, 这边与规划那边对应
