...
#pragma omp for
  for (size_t i = 0; i < in_radial_ordered_clouds.size(); i++)  // sweep through each radial division
  {
    ...
    for (size_t j = 0; j < in_radial_ordered_clouds[i].size(); j++)  // loop through each point in the radial div
    {
      ...
      float general_height_threshold = tan(DEG2RAD(general_max_slope_)) * in_radial_ordered_clouds[i][j].radius; // weiming  add: 这里需要调参

      // for points which are very close causing the height threshold to be tiny, set a minimum value
      if (points_distance > concentric_divider_distance_ && height_threshold < min_height_threshold_)
      {
        height_threshold = min_height_threshold_;   // 给局部高度阈值一个最小值
      }

      // check current point height against the LOCAL threshold (previous point)
      if (current_height <= (prev_height + height_threshold) && current_height >= (prev_height - height_threshold))  // 如果这个点在之前点的一个小角度(local_max_slope_)范围内
      {
        // Check again using general geometry (radius from origin) if previous points wasn't ground
        if (!prev_ground)   // 如果之前的点不是地面点, 那么只有当这个点的高度在全局小角度范围内才算地面点
        {
          if (current_height <= general_height_threshold  && current_height >= -general_height_threshold)   // ljc add: 这里说明了为什么base_frame要选base_link(需要离地面较近的一个link)
          {
            current_ground = true;
          }
          else
      ...
      else
      {
        ...
        if (points_distance > reclass_distance_threshold_ &&
            (current_height <= height_threshold && current_height >= -height_threshold))   // 如果当前点不在之前点小角度内, 但距离较远, 感觉非常依赖于base_frame, 吊用没有
        {
          current_ground = true;
        }
        else
...
void RayGroundFilter::RemovePointsToSize(const pcl::PointCloud<pcl::PointXYZI>::Ptr in_cloud_ptr, double front_x_size, double back_x_size, double half_y_size,
...
#pragma omp for
  for (size_t i = 0; i < in_cloud_ptr->points.size(); i++)
  {
    if (in_cloud_ptr->points[i].x < front_x_size && in_cloud_ptr->points[i].x > -back_x_size
          && in_cloud_ptr->points[i].y < half_y_size && in_cloud_ptr->points[i].y > -half_y_size)
    {
      indices.indices.push_back(i);
    }
  }
...
void RayGroundFilter::CloudCallback(const sensor_msgs::PointCloud2ConstPtr& in_sensor_cloud)
{
  ...
  // ljc add: 去除一些为Nan值的点云, 最好是加在
  pcl::PointCloud<pcl::PointXYZI>::Ptr no_nan_points_ptr(new pcl::PointCloud<pcl::PointXYZI>);
  std::vector<int> mapping;
  pcl::removeNaNFromPointCloud(*current_sensor_cloud_ptr, *no_nan_points_ptr, mapping);
  pcl::PointCloud<pcl::PointXYZI>::Ptr clipped_cloud_ptr(new pcl::PointCloud<pcl::PointXYZI>);

  // remove points above certain point
  ClipCloud(no_nan_points_ptr, clipping_height_, clipped_cloud_ptr);

  // remove closer points than a threshold
  pcl::PointCloud<pcl::PointXYZI>::Ptr filtered_cloud_ptr(new pcl::PointCloud<pcl::PointXYZI>);
  pcl::PointCloud<pcl::PointXYZI>::Ptr in_filtered_cloud_ptr(new pcl::PointCloud<pcl::PointXYZI>);
  // RemovePointsUpTo(clipped_cloud_ptr, min_point_distance_, filtered_cloud_ptr);
  RemovePointsToSize(clipped_cloud_ptr, front_x_size_, back_x_size_, half_y_size_, filtered_cloud_ptr, true);   // true是要去掉这个范围内的点, false是保留这个范围内的点, 这里是去除车身的点, 所以用true 
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr no_ground_cloud_exact_ptr(new pcl::PointCloud<pcl::PointXYZI>);

  ExtractPointsIndices(filtered_cloud_ptr, *ground_indices, ground_cloud_ptr, no_ground_cloud_ptr);

  *no_ground_cloud_exact_ptr = *no_ground_cloud_ptr;

  // ljc add: 发送地面方程, 这里的地面方程是在base_link坐标系下的, 需要转换到camera坐标系, subscribe将在range_vision_fusion中做
  // 这部分地面提取用的不是pcl中的ransac, 并且实测效果更好, 所以提取完地面点后再将其计算一次平面方程
  ...
  seg.setOptimizeCoefficients(true);
  seg.setInputCloud(ground_cloud_ptr);
  ...
  // ROS_INFO_STREAM(current - past);
  std::vector<float> plane_coeff = {coefficients->values[0], coefficients->values[1], coefficients->values[2], coefficients->values[3]};
  std_msgs::Float32MultiArray plane_coeff_msgs;
  plane_coeff_msgs.data = plane_coeff;
  _pub_plane_coeff.publish(plane_coeff_msgs);   

  RemovePointsToSize(ground_cloud_ptr, front_range_x_size_, back_range_x_size_, range_y_size_, ground_cloud_ptr, false);   // true是要去掉这个范围内的点, false是保留这个范围内的点, 这里是去除太远的点云, 所以要保留
  RemovePointsToSize(no_ground_cloud_ptr, front_range_x_size_, back_range_x_size_, range_y_size_, no_ground_cloud_ptr, false);   // true是要去掉这个范围内的点, false是保留这个范围内的点, 这里是去除太远的点云, 所以要保留
  ...
  publish_cloud(groundless_exact_points_pub_, no_ground_cloud_exact_ptr, in_sensor_cloud->header);

  visualization_msgs::MarkerArray range_markers;
  ShowRectFilteredArea(front_x_size_, back_x_size_, half_y_size_, range_markers, 0);   // 内圈的marker
  ShowRectFilteredArea(front_range_x_size_, back_range_x_size_, range_y_size_, range_markers, 1);   // 外圈的marker
  ray_ground_rect_pub.publish(range_markers);
}
...
void RayGroundFilter::ShowRectFilteredArea(double front_x_size, double back_x_size, double half_y_size, visualization_msgs::MarkerArray& markers, int id)
{
  //if(safety_rect.size() != 4) return;

  visualization_msgs::Marker lane_waypoint_marker;
  lane_waypoint_marker.header.frame_id = base_frame_;
  lane_waypoint_marker.header.stamp = ros::Time();
  lane_waypoint_marker.ns = "ray_ground_filter";
  ...
  markers.markers.emplace_back(lane_waypoint_marker);
}
...
void RayGroundFilter::Run()
{
  ...
  node_handle_.param("front_vehicle_x_size", front_x_size_, 1.3);  
  ROS_INFO("front_vehicle_x_size[meters]: %f", front_x_size_);
  node_handle_.param("back_vehicle_x_size", back_x_size_, 1.0);  
  ROS_INFO("back_vehicle_x_size[meters]: %f", back_x_size_);
  node_handle_.param("half_vehicle_y_size", half_y_size_, 0.8);  
  ROS_INFO("half_vehicle_y_size[meters]: %f", half_y_size_);

  node_handle_.param("range_y_size", range_y_size_, 5.0);  
  ROS_INFO("range_y_size[meters]: %f", range_y_size_);
  node_handle_.param("front_range_x_size", front_range_x_size_, 10.0);  
  ROS_INFO("front_range_x_size[meters]: %f", front_range_x_size_);
  node_handle_.param("back_range_x_size", back_range_x_size_, 5.0);  
  ROS_INFO("back_range_x_size[meters]: %f", back_range_x_size_);
  ...
  groundless_exact_points_pub_ = node_handle_.advertise<sensor_msgs::PointCloud2>("/points_no_ground_exact", 2);
  ground_points_pub_ = node_handle_.advertise<sensor_msgs::PointCloud2>(ground_topic, 2);
  _pub_plane_coeff = node_handle_.advertise<std_msgs::Float32MultiArray>("/points_ground_coeff", 1);

  ray_ground_rect_pub = node_handle_.advertise<visualization_msgs::MarkerArray>("/ray_ground_rect", 1);
