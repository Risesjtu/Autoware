...
PointsConcatFilter::PointsConcatFilter() : node_handle_(), private_node_handle_("~"), tf_listener_()
{
  ...
  private_node_handle_.param("need_sample", need_sample_, false);
                                             ...
                                             const PointCloudMsgT::ConstPtr &msg7, const PointCloudMsgT::ConstPtr &msg8)
{
  ...
  pcl::removeNaNFromPointCloud(*cloud_concatenated, *no_nan_points_ptr, mapping);
  
  if (need_sample_)
  {
    pcl::PointCloud<pcl::PointXYZI>::Ptr downsample_points_ptr(new pcl::PointCloud<pcl::PointXYZI>);
    pcl::VoxelGrid<pcl::PointXYZI> sor;
    sor.setInputCloud(no_nan_points_ptr);
    sor.setLeafSize(0.11f, 0.11f, 0.11f);//设置滤波时创建的体素体积为1 cm3的立方体
    sor.filter(*downsample_points_ptr);

    // publsh points
    downsample_points_ptr->header = pcl_conversions::toPCL(msgs[0]->header);
    downsample_points_ptr->header.frame_id = output_frame_id_;
    cloud_publisher_.publish(downsample_points_ptr);
  }
  else
  {
    // publsh points
    no_nan_points_ptr->header = pcl_conversions::toPCL(msgs[0]->header);
    no_nan_points_ptr->header.frame_id = output_frame_id_;
    cloud_publisher_.publish(no_nan_points_ptr);
  }
}
