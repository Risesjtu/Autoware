...
private:
  ...
  ros::Publisher groundless_exact_points_pub_;
  ros::Publisher ground_points_pub_;
  ros::Publisher _pub_plane_coeff;    // ljc add for monocular distance prediction
  ros::Publisher ray_ground_rect_pub;
  ...
  double front_x_size_;
  double back_x_size_;
  double half_y_size_;
  double range_y_size_;
  double front_range_x_size_;
  double back_range_x_size_;
  ...
  void RemovePointsToSize(const pcl::PointCloud<pcl::PointXYZI>::Ptr in_cloud_ptr, double front_x_size, double back_x_size, double half_y_size,
                                       pcl::PointCloud<pcl::PointXYZI>::Ptr out_filtered_cloud_ptr, bool keep);

  void CloudCallback(const sensor_msgs::PointCloud2ConstPtr& in_sensor_cloud);

  void ShowRectFilteredArea(double front_x_size, double back_x_size, double half_y_size, visualization_msgs::MarkerArray& markers, int id);
